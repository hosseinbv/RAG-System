"""Regenerate architecture and LangGraph topology diagrams."""
...
SYSTEM_GRAPH_DOT = """digraph LangGraphRagSystem {
  graph [
    rankdir=TB,
    bgcolor="white",
...
def default_output_dir() -> Path:
    return project_root() / "reports" / "graphs"


def write_system_graph_dot(out_dir: Path) -> Path:
    path = out_dir / "system_graph.dot"
    path.write_text(SYSTEM_GRAPH_DOT, encoding="utf-8")
    return path


def render_graphviz(dot_path: Path, out_dir: Path) -> dict[str, bool]:
    dot = shutil.which("dot")
    rendered = {"system_graph.svg": False, "system_graph.png": False}
    if dot is None:
        return rendered
    for fmt in ("svg", "png"):
        out_path = out_dir / f"system_graph.{fmt}"
        subprocess.run([dot, f"-T{fmt}", str(dot_path), "-o", str(out_path)], check=True)
        rendered[out_path.name] = True
    return rendered


def compiled_mermaid(web_enabled: bool) -> str:
    cfg = Config(deepcopy(load_config()))
    cfg.setdefault("web", {})["enabled"] = web_enabled
    return build_pipeline(cfg).get_graph().draw_mermaid()


def write_langgraph_mermaid(out_dir: Path) -> dict[str, Path]:
    outputs = {
        "langgraph_corpus_only.mmd": compiled_mermaid(False),
        "langgraph_web_augmented.mmd": compiled_mermaid(True),
    }
    paths: dict[str, Path] = {}
...
def render_all(out_dir: str | Path | None = None) -> dict[str, object]:
    target = Path(out_dir) if out_dir else default_output_dir()
    target.mkdir(parents=True, exist_ok=True)
    dot_path = write_system_graph_dot(target)
    mermaid_paths = write_langgraph_mermaid(target)
    rendered = render_graphviz(dot_path, target)
    return {
        "out_dir": str(target),
        "system_graph_dot": str(dot_path),
        "mermaid": {name: str(path) for name, path in mermaid_paths.items()},
        "rendered": rendered,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Directory for graph artifacts. Defaults to reports/graphs.",
    )
    args = parser.parse_args(argv)
    result = render_all(args.out_dir)
    print(f"wrote graph artifacts to {result['out_dir']}")
    for name, ok in result["rendered"].items():
        print(f"{name}: {'rendered' if ok else 'skipped; graphviz dot not found'}")
    return 0
